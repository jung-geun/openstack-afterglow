...
<div class="p-4 md:p-8">
  ...
  {:else}
    <div class="overflow-x-auto">
      <div class="min-w-[600px] bg-[#0B1220] border border-gray-800 rounded-[10px] overflow-hidden">
        <!-- 헤더 -->
        <div class="grid grid-cols-[1.5fr_90px_1fr_1.2fr_90px_90px_100px] px-4 py-2.5 border-b border-gray-800 text-[11px] uppercase tracking-wider text-gray-500 font-medium">
          <div>이름</div>
          <div>상태</div>
        ...
        {#each instances as inst (inst.id)}
          <div
            ...
            tabindex="0"
            role="button"
            class="grid grid-cols-[1.5fr_90px_1fr_1.2fr_90px_90px_100px] px-4 py-3 text-[13px] items-center border-b border-gray-800 hover:bg-gray-800/30 transition-colors cursor-pointer last:border-b-0"
          >
            <!-- 이름 -->
            ...
            <!-- 상태 -->
            <div class="hidden sm:block"><StatusChip status={inst.status} /></div>
            <!-- 이미지/플레이버 -->
            <div class="hidden md:block text-xs">
              <div class="text-gray-300 truncate">{inst.image_name ?? '볼륨에서 부팅'}</div>
              {#if inst.flavor_name}<div class="text-gray-600 mt-0.5 truncate">{inst.flavor_name}</div>{/if}
            </div>
            <!-- IP -->
            <div class="text-xs">
            ...
            <div class="flex items-center justify-end gap-1" onclick={(e) => e.stopPropagation()} role="none">
              ...
              {#if inst.status === 'SHELVED_OFFLOADED' || inst.status === 'SHELVED'}
                <button onclick={(e) => { e.stopPropagation(); unshelveInstance(inst.id); }} class="text-green-400 hover:text-green-300 text-xs px-2 py-1 rounded border border-green-900 hover:border-green-700 transition-colors hidden sm:inline-flex">해제</button>
              {/if}
