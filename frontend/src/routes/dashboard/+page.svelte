<script lang="ts">
	...
	type Tab = 'instances' | 'volumes' | 'shares' | 'networks' | 'routers' | 'loadbalancers' | 'security-groups';
	...
	if (tab === 'security-groups') return securityGroups.length;
	...
	async function fetchSecurityGroups() {
		const path = '/api/security-groups';
		const cached = swrGet<SecurityGroup[]>(path);
		if (cached && securityGroups.length === 0) securityGroups = cached;
		try {
			securityGroups = await api.get<SecurityGroup[]>(path, $auth.token ?? undefined, $auth.projectId ?? undefined);
			swrSet(path, securityGroups);
			sgError = '';
		} catch (e) {
			if (!cached) sgError = e instanceof ApiError ? `조회 실패 (${e.status}): ${(e as ApiError).message}` : '서버 오류';
		}
	}
	...
	function openModal(tab: Tab) {
		...
		else if (tab === 'security-groups') { showSgModal = true; sgCreateError = ''; }
	}
	...
	function formatSgRule(r: SecurityGroupRule): string {
...
<div class="p-8">
	...
	{:else if activeTab === 'security-groups'}
		{#if loading}
			<LoadingSkeleton variant="table" rows={5} />
		{:else if sgError}
			<div class="text-red-400 text-sm py-8 text-center">{sgError}</div>
