<script lang="ts">
	...
	let moveError = $state('');
	let moveContainers = $state<ContainerInfo[]>([]);
	...
	async function loadContainerMeta() {
		try {
			containerMeta = await api.get<ContainerMeta>(
				`/api/object-storage/${encodeURIComponent(containerName)}`, token, projectId
			);
	...
	function encObj(name: string) { return name.split('/').map(encodeURIComponent).join('/'); }
	function breadcrumbPrefix(idx: number) { return breadcrumbs.slice(0, idx + 1).join('/') + '/'; }

	function navigatePrefix(newPrefix: string) {
		prefix = newPrefix; selectedMeta = null; showPreview = false; filterText = '';
		selected = new Set();
		load();
	}
	...
	async function downloadObject(name: string) {
		...
		try {
			const { blob, filename } = await api.downloadBlob(
				`/api/object-storage/${encodeURIComponent(containerName)}/objects/${encObj(name)}/download`, token, projectId
			);
			const url = URL.createObjectURL(blob);
			const a = document.createElement('a');
			a.href = url; a.download = filename; a.click();
			URL.revokeObjectURL(url);
		} catch (e) { alert('다운로드 실패: ' + (e instanceof ApiError ? e.message : String(e))); }
	...
	async function deleteObject(name: string) {
		...
		deleting = name;
		try {
			await api.delete(`/api/object-storage/${encodeURIComponent(containerName)}/objects/${encObj(name)}`, token, projectId);
			await load();
	...
	async function showMeta(name: string) {
		...
		try {
			selectedMeta = await api.get<ObjectMeta>(
				`/api/object-storage/${encodeURIComponent(containerName)}/objects/${encObj(name)}/metadata`, token, projectId
			);
		} catch { /* ignore */ }
	...
	async function openPreview(obj: ObjectItem) {
		selectedMeta = null; showPreview = true; previewName = obj.name;
		previewContentType = obj.content_type; previewText = ''; previewUrl = '';
		loadingPreview = true;
	...
	async function createDirectory() {
		...
		try {
			await api.post(`/api/object-storage/${encodeURIComponent(containerName)}/objects/directory`,
				{ path: prefix + newDirName.trim().replace(/\/$/, '') + '/' }, token, projectId);
			showNewDir = false; newDirName = ''; await load();
		} catch (e) { dirError = e instanceof ApiError ? e.message : '폴더 생성 실패'; }
	...
	async function doRename() {
		...
		try {
			...
			await api.post(`/api/object-storage/${encodeURIComponent(containerName)}/objects/rename`,
				{ source: renameTarget, new_name: newFullName }, token, projectId);
			showRename = false; await load();
		} catch (e) { renameError = e instanceof ApiError ? e.message : '이름 변경 실패'; }
		finally { renaming = false; }
	}

	async function loadMoveContainers() {
		try { moveContainers = await api.get<ContainerInfo[]>('/api/object-storage', token, projectId); }
	...
	async function loadMoveDirectories(targetContainer: string) {
		...
		try {
			const all = await api.get<ObjectItem[]>(
				`/api/object-storage/${encodeURIComponent(targetContainer)}/objects?delimiter=/`, token, projectId);
	...
	async function openMove(name: string) {
		moveTarget = name; moveDest = ''; moveDestContainer = containerName;
	...
	async function doMove() {
		...
		try {
			await api.post(`/api/object-storage/${encodeURIComponent(containerName)}/objects/move`,
				{ source: moveTarget, destination: moveDest.trim(), dest_container: moveDestContainer || null },
				token, projectId);
			showMove = false; await load();
		} catch (e) { moveError = e instanceof ApiError ? e.message : '이동 실패'; }
		finally { moving = false; }
	}

	async function openBulkMove() {
		moveDest = ''; moveDestContainer = containerName;
...
<div class="p-4 md:p-8 max-w-7xl">
	...
	<div class="flex items-center gap-1 mb-2 flex-wrap text-sm">
		<a href="/admin/object-storage" class="text-gray-500 hover:text-gray-300 flex items-center gap-1">
	...
	<div class="flex items-start justify-between mb-4">
		<div>
			...
			{#if containerMeta}<p class="text-gray-500 text-xs mt-1">데이터 동기화 완료</p>{/if}
		</div>
		{#if containerMeta}
			<div class="flex gap-4 text-center">
				<div class="bg-gray-900 border border-gray-800 rounded-lg px-4 py-2">
					...
					<div class="text-lg font-bold text-white">
						{containerMeta.bytes >= 1073741824 ? (containerMeta.bytes / 1073741824).toFixed(2)
							: containerMeta.bytes >= 1048576 ? (containerMeta.bytes / 1048576).toFixed(1)
	...
	<div class="flex items-center gap-2 mb-4 flex-wrap">
		<div class="relative flex-1 min-w-[200px] max-w-xs">
			...
			<input type="text" bind:value={filterText} placeholder="파일 필터..."
		...
		<button onclick={() => toggleSort('name')}
			class="text-xs text-gray-400 hover:text-white px-3 py-1.5 rounded border border-gray-700 hover:border-gray-600 transition-colors">이름순 {sortIcon('name')}</button>
		<div class="flex-1"></div>
		{#if prefix}
			<button onclick={() => { const p = prefix.replace(/\/$/, '').split('/'); p.pop(); navigatePrefix(p.length ? p.join('/') + '/' : ''); }}
				class="text-xs text-gray-400 hover:text-white transition-colors px-3 py-1.5 rounded border border-gray-700 hover:border-gray-600">← 상위</button>
		{/if}
		...
		<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
			<div class="bg-gray-900 border border-gray-800 rounded-xl p-6 w-full max-w-sm shadow-2xl">
				...
				<div class="flex gap-2 justify-end">
					<button onclick={() => { showNewDir = false; }} class="text-xs text-gray-400 hover:text-white px-3 py-1.5 rounded border border-gray-700">취소</button>
	...
	{#if showRename}
		<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
			<div class="bg-gray-900 border border-gray-800 rounded-xl p-6 w-full max-w-sm shadow-2xl">
				...
				<div class="flex gap-2 justify-end">
					<button onclick={() => { showRename = false; }} class="text-xs text-gray-400 hover:text-white px-3 py-1.5 rounded border border-gray-700">취소</button>
					<button onclick={doRename} disabled={renaming}
						class="text-xs text-white bg-indigo-600 hover:bg-indigo-500 disabled:bg-gray-700 px-3 py-1.5 rounded border border-indigo-500 disabled:border-gray-600">
						{renaming ? '변경 중...' : '변경'}</button>
				</div>
			</div>
		</div>
	{/if}
		...
		<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/70"
			...
			<div class="bg-gray-900 border border-gray-800 rounded-xl p-6 w-full max-w-md shadow-2xl"
				...
				{#if moveContainers.length > 0}
					<select value={moveDestContainer}
						...
						{#each moveContainers as c}<option value={c.name}>{c.name} ({c.count} items)</option>{/each}
					</select>
				{:else}
				...
				{#if moveLoadingDirs}
					<div class="bg-gray-800 border border-gray-700 rounded-lg p-3 mb-3"><p class="text-gray-500 text-xs">디렉토리 목록 로딩 중...</p></div>
				{:else}
					<div class="bg-gray-800 border border-gray-700 rounded-lg max-h-48 overflow-y-auto mb-3">
						{#each moveDirectories.filter(d => d === '/ (루트)' || (d !== moveTarget && !d.startsWith(moveTarget))) as dir}
							...
							<button onclick={() => selectMoveDir(dir)}
	...
	{#if showBulkMove}
		<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/70"
			...
			<div class="bg-gray-900 border border-gray-800 rounded-xl p-6 w-full max-w-md shadow-2xl"
				...
				<label class="text-gray-400 text-xs mb-1 block font-medium">대상 디렉토리</label>
				{#if moveLoadingDirs}
					<div class="bg-gray-800 border border-gray-700 rounded-lg p-3 mb-3"><p class="text-gray-500 text-xs">로딩 중...</p></div>
				{:else}
					<div class="bg-gray-800 border border-gray-700 rounded-lg max-h-48 overflow-y-auto mb-3">
						{#each moveDirectories.filter(d => d === '/ (루트)' || ![...selected].some(s => d === s || d.startsWith(s))) as dir}
							{@const isSelected = (dir === '/ (루트)' && moveSelectedDir === '') || dir === moveSelectedDir}
							<button onclick={() => { const filename = ''; if (dir === '/ (루트)') { moveSelectedDir = ''; moveDest = filename; } else { moveSelectedDir = dir; moveDest = dir; } }}
								class="w-full text-left px-3 py-2 text-sm flex items-center gap-2 transition-colors {isSelected ? 'bg-indigo-600/20 text-indigo-300 border-l-2 border-indigo-500' : 'text-gray-300 hover:bg-gray-700/50'}">
								<svg class="w-4 h-4 shrink-0 {isSelected ? 'text-indigo-400' : 'text-amber-400'}" viewBox="0 0 20 20" fill="currentColor"><path d="M2 6a2 2 0 012-2h4l2 2h6a2 2 0 012 2v6a2 2 0 01-2 2H4a2 2 0 01-2-2V6z"/></svg>
								<span class="truncate">{dir}</span>
								{#if isSelected}<svg class="w-4 h-4 ml-auto text-indigo-400 shrink-0" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>{/if}
							</button>
						{/each}
					</div>
				{/if}
				<div class="flex gap-2 justify-end">
					<button onclick={() => { showBulkMove = false; moveError = ''; }}
						class="text-xs text-gray-400 hover:text-white px-4 py-2 rounded-lg border border-gray-700 transition-colors">취소</button>
					<button onclick={doBulkMove} disabled={bulkMoving}
						class="text-xs text-white bg-indigo-600 hover:bg-indigo-500 disabled:bg-gray-700 disabled:text-gray-500 px-4 py-2 rounded-lg border border-indigo-500 disabled:border-gray-600 transition-colors">
	...
	<div class="flex gap-6">
						...
						<thead>
							<tr class="border-b border-gray-800 text-gray-500 text-xs uppercase tracking-wide">
								...
								<th class="text-left py-3 px-4 font-medium relative">
									...
									<div class="absolute right-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-indigo-500/40 active:bg-indigo-500/60" onmousedown={(e) => onResizeStart(e, 'name')}></div>
								</th>
								<th class="text-left py-3 px-4 font-medium relative">
									<button onclick={() => toggleSort('bytes')} class="hover:text-gray-300 transition-colors">크기 {sortIcon('bytes')}</button>
									<div class="absolute right-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-indigo-500/40 active:bg-indigo-500/60" onmousedown={(e) => onResizeStart(e, 'bytes')}></div>
								</th>
								<th class="text-left py-3 px-4 font-medium relative">
									타입
									<div class="absolute right-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-indigo-500/40 active:bg-indigo-500/60" onmousedown={(e) => onResizeStart(e, 'type')}></div>
								</th>
								<th class="text-left py-3 px-4 font-medium relative">
									<button onclick={() => toggleSort('last_modified')} class="hover:text-gray-300 transition-colors">수정일 {sortIcon('last_modified')}</button>
									<div class="absolute right-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-indigo-500/40 active:bg-indigo-500/60" onmousedown={(e) => onResizeStart(e, 'modified')}></div>
								</th>
								...
								>
									...
									<td class="py-3 px-4">
										<div class="flex items-center gap-2.5">
											...
											{#if isDir}
												<button onclick={() => navigatePrefix(obj.name)}
													class="text-white hover:text-indigo-300 text-left truncate max-w-md font-medium hover:underline">{relName || obj.name}</button>
											{:else}
												<button onclick={() => showMeta(obj.name)}
													class="text-gray-200 hover:text-white text-left truncate max-w-md hover:underline">{relName || obj.name}</button>
											{/if}
										</div>
									</td>
									<td class="py-3 px-4 text-gray-400 whitespace-nowrap">
										{isDir ? '-' : obj.bytes >= 1073741824 ? formatStorage(Math.round(obj.bytes / 1073741824))
									...
									<td class="py-3 px-4 text-right">
										<div class="flex items-center justify-end gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
											{#if !isDir && isPreviewable(obj.content_type)}
												<button onclick={() => openPreview(obj)} title="미리보기" class="p-1.5 text-gray-400 hover:text-white hover:bg-gray-700 rounded transition-colors">
											...
											{/if}
											{#if !isDir}
												<button onclick={() => downloadObject(obj.name)} title="다운로드" class="p-1.5 text-gray-400 hover:text-white hover:bg-gray-700 rounded transition-colors">
											...
											{/if}
											<button onclick={() => openRename(obj.name)} title="이름변경" class="p-1.5 text-gray-400 hover:text-white hover:bg-gray-700 rounded transition-colors">
											...
											<button onclick={() => openMove(obj.name)} title="이동" class="p-1.5 text-gray-400 hover:text-white hover:bg-gray-700 rounded transition-colors">
											...
											<button onclick={() => deleteObject(obj.name)} title="삭제" class="p-1.5 text-red-400 hover:text-red-300 hover:bg-gray-700 rounded transition-colors">
		...
		{#if loadingMeta}
			<div class="w-72 shrink-0 bg-gray-900 border border-gray-800 rounded-xl p-4"><LoadingSkeleton variant="detail" rows={6} /></div>
		...
		{:else if showPreview}
			<div class="w-96 shrink-0 bg-gray-900 border border-gray-800 rounded-xl p-4 text-sm flex flex-col">
				...
				{#if loadingPreview}<div class="text-gray-500 text-xs">로딩 중...</div>
				{:else if previewContentType.startsWith('image/')}<img src={previewUrl} alt={previewName} class="max-w-full rounded object-contain max-h-96" />
				{:else if previewContentType === 'application/pdf'}<embed src={previewUrl} type="application/pdf" class="w-full h-96 rounded" />
				{:else}<pre class="text-gray-300 text-xs overflow-auto max-h-96 whitespace-pre-wrap break-all bg-gray-950 rounded p-2">{previewText}</pre>
				{/if}
			</div>
		{/if}
	</div>
