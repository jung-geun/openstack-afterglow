<script lang="ts">
	...
	const compactProgressPct = $derived((cur / Math.max(1, totalSteps)) * 100);
</script>

<div class="stepper-container">
	<div class="full-stepper relative bg-gray-900 border border-gray-800 rounded-2xl px-6 py-4 mb-6 max-[479px]:hidden">
	...
	<div
		class="compact-stepper hidden mb-6 rounded-xl border border-[var(--color-line)] bg-[var(--color-surface-base)] px-4 py-3 max-[479px]:block"
		role="progressbar"
		aria-label="VM 생성 진행률"
		aria-valuemin="1"
		aria-valuemax={totalSteps}
		aria-valuenow={cur}
	>
		<div class="flex items-center justify-between gap-3 text-[11px] font-mono uppercase tracking-[0.06em] text-[var(--color-ink-2)]">
			<span>STEP {cur} / {totalSteps}</span>
			<span class="truncate text-right text-[var(--color-ink-1)]">{stepLabels[cur - 1]}</span>
		</div>
		<div class="mt-2 h-1.5 overflow-hidden rounded-full bg-[var(--color-surface-sunken)]">
			<div class="compact-progress-fill h-full rounded-full transition-[width] duration-300" style="width: {compactProgressPct}%"></div>
		</div>
	</div>
</div>
