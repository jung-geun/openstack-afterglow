						...
						<td class="py-3 px-4">
							<div class="flex items-center gap-2.5" style="padding-left: {row.depth * 16}px">
								...
								{#if isDir && !row.fullPath}
									<button onclick={() => s.navigatePrefix(obj.name)} class="text-white hover:text-indigo-300 text-left truncate max-w-md font-medium hover:underline">{rowLabel}</button>
								{:else}
									<button onclick={() => s.showMeta(obj.name)} class="text-gray-200 hover:text-white text-left truncate max-w-md hover:underline {row.fullPath ? 'font-mono text-xs' : ''}">{rowLabel}</button>
								{/if}
							</div>
						</td>
