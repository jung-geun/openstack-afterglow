...
Backport catalog-relative path normalization and its regression test onto the service-owned Drover SDK commit currently consumed by Afterglow. Align Drover's `FlavorInfo` with every live caller so GPU extra specs survive quota usage calculation. Pin Afterglow to the resulting immutable Drover commit and refresh the lockfile. Do not bypass GPU quota checks, hardcode the live endpoint, or migrate the service catalog as part of this incident fix.
...
- The live GPU quota check succeeds and the original VM creation progresses past quota validation.
