...
- [x] Preserve shared external-MCP authentication mode ownership: a user OAuth start may promote only their own server to OAuth, never mutate an active global server and suppress other users’ configured manual credentials. Evidence: `backend/tests/test_chat_mcp_oauth.py` (10 passed).
- [x] Add explicit external-MCP authentication modes (`none`, `headers`, `oauth`) and an administrator-only static OAuth client fallback with encrypted secrets and declared scopes; retain DCR when no static client is configured. Evidence: migration `068_chat_mcp_auth_configuration.sql`, `backend/tests/test_chat_mcp_config.py`, and `backend/tests/test_chat_mcp_oauth.py` (2026-07-28).
- [x] Add user/project-scoped encrypted MCP environment variables, explicit header-template bindings, and deterministic credential-source precedence without ever injecting user values into the API process environment. Evidence: `backend/tests/test_chat_mcp_config.py`, `backend/tests/test_chat_tool_runtime.py` (2026-07-28).
- [x] Expose the administrator OAuth configuration and user secret-variable/OAuth-connect surfaces, then cover migration, ownership, secret masking, OAuth fallback, mode gating, and UI contracts with focused tests. Evidence: `backend/tests/test_chat_extensions.py`, `frontend/src/lib/components/chat/__tests__/ChatExtensionsManager.test.ts`, and `backend/tests/test_migration_ledger.py` (2026-07-28).
...
- [x] Cut over external MCP sources to public HTTPS Streamable HTTP or detected OAuth only; remove user header/template credentials, route remote execution through pinned LangChain MCP adapters, require administrator configuration for static authentication, bind OAuth state/tokens to MCP configuration revisions, and deactivate incompatible legacy header sources in migration `069`. Evidence: `backend/tests/test_chat_mcp_client.py`, `backend/tests/test_chat_mcp_config.py`, `backend/tests/test_chat_mcp_oauth.py`, `backend/tests/test_chat_extensions.py`, `backend/tests/test_chat_tool_runtime.py`, `backend/tests/test_chat_extensions_store.py`, `backend/tests/test_migration_ledger.py`, `frontend/src/lib/components/chat/__tests__/ChatExtensionsManager.test.ts` (2026-07-28).
