...
- [x] Verify repository Kolla syntax and lifecycle task lists.
- [x] Run complete repository test and backend lint gates.
- [ ] Install, baseline, deploy, reconfigure, and prove the isolated DMSLab plugin deployment.
  - Blocked before database/Keystone mutation: DMSLab already owns the internal VIP's port 8000 and stock Valkey binds API-interface addresses rather than the VIP. An explicit revised topology is required by the approved plan.