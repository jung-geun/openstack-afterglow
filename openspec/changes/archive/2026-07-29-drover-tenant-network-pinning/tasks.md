...
- [x] Run `npm run test:all` and `npm run lint:backend` successfully.
- [x] Complete the available live OpenStack smoke proof or record the exact unavailable prerequisite.
- [x] Archive with `openspec archive drover-tenant-network-pinning --skip-specs --yes` after verification.
...
Full-gate note: `npm run test:all` passed with backend (3735 passed, 61 skipped),
integration (199 passed, 13 skipped), and frontend (860 passed) suites. `npm run
lint:backend` passed with Ruff checks clean and 658 files already formatted.
