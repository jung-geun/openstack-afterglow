...
- Reconfigure the live Drover deployment and verify readiness before running staging integration.
...
Drover Kolla deploy/reconfigure resolves one existing Keystone project and recreates containers only when the rendered configuration changes. Missing or ambiguous `drover-service` projects fail before rendering instead of producing a live-but-unready API.
