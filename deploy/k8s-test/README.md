...
```bash
docker build -t afterglow-backend:latest ./backend
docker build -t afterglow-frontend:latest ./frontend
```
...
```bash
# minikube 사용 시
minikube image load afterglow-backend:latest
minikube image load afterglow-frontend:latest
```
