#!/usr/bin/env python3
"""Validate immutable Palimpsest OCI image references before deployment mutation."""
...
_LOCAL_SOURCE_REF = re.compile(r"^afterglow-local/palimpsest-hub-(?:api|worker):[0-9a-f]{12}$")
...
def validate_image_ref(reference: str, *, source_mode: bool) -> None:
    ...
    if not pattern.fullmatch(reference):
        expected = (
            "afterglow-local/palimpsest-hub-{api|worker}:<12-hex-commit>"
            if source_mode
            else "registry/repository@sha256:<64-lowercase-hex>"
        )
        raise ValueError(f"invalid Palimpsest image reference; expected {expected}")
