...
async def test_list_namespaces_cluster_not_found(client):
    with (
        patch("app.api.k3s.configmaps.k3s_cluster") as mock_cluster,
        patch("app.api.k3s.configmaps.k3s_kube") as mock_kube,
    ):
...
async def test_create_configmap_success(client):
    with (
        patch("app.api.k3s.configmaps.k3s_cluster") as mock_cluster,
        patch("app.api.k3s.configmaps.k3s_kube") as mock_kube,
        patch("app.api.k3s.configmaps.rec", new_callable=AsyncMock),
        patch("app.api.k3s.configmaps.invalidate", new_callable=AsyncMock),
    ):
        mock_cluster.get_cluster = AsyncMock(return_value=_make_cluster_record())
