...
from app.api.ai_compat import anthropic as an
from app.api.ai_compat import openai as oa
from app.services.chat import api_key_store as aks
from app.services.chat import completion_api as core
...
def _allow_all_hosts(monkeypatch):
    ...
    monkeypatch.setattr("app.api.deps.get_settings", lambda: SimpleNamespace(chat_api_host_list=[]))
...
class TestDiscoveryAndHostGate:
    ...
    async def test_blocked_host_404(self, client, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.api.deps.get_settings", lambda: SimpleNamespace(chat_api_host_list=["api.cloud.example"])
        )
