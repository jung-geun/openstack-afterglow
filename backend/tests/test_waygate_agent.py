...
from app.services import waygate_agent_auth, waygate_config
...
_AGENT_ENDPOINTS = [
    ("post", "/api/v1/waygate/servers/server-1/agent/register", {"public_key": "A" * 43 + "="}),
    ("get", "/api/v1/waygate/servers/server-1/agent/desired-state", None),
    ("post", "/api/v1/waygate/servers/server-1/agent/status", {"peers": []}),
]
...
class TestAgentRegisterHappyPath:
    ...
    async def test_valid_token_register_updates_public_key_and_status(self, api_client):
        token = await waygate_agent_auth.issue_report_token("server-1", "test-project-123")
        with patch("app.api.waygate.agent.waygate_db") as mock_db:
