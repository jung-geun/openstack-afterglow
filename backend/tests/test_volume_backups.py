...
async def test_list_backups(client, mock_conn):
    with patch("app.api.volume_backups.cinder.list_backups", return_value=[make_backup()]):
...
async def test_create_backup(client, mock_conn):
    with patch("app.api.volume_backups.cinder.create_backup", return_value=make_backup("backup-new")):
...
async def test_delete_backup(client, mock_conn):
    with patch("app.api.volume_backups.cinder.delete_backup", return_value=None):
