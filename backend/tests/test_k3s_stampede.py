...
def test_assign_pending_pods_single_best_nodegroup_for_no_selector():
    from app.services.k3s_stampede import _assign_pending_pods
...
def test_binpack_counts_gpu_memory_and_cpu():
    from app.services.k3s_stampede import _binpack_count
...
async def test_scale_up_gpu_pending_pod_ignores_cpu_only_free_capacity():
    ...
    with (
        ...
        patch(
            "app.services.k3s_stampede._get_available_flavors",
            new=AsyncMock(
                return_value=[{"id": "gpu", "name": "gpu.large", "vcpus_m": 4000, "ram_bytes": 8 * 1024**3, "gpu": 1}]
            ),
        ),
        patch("app.services.k3s_stampede._check_gpu_quota_for_nodes", new=AsyncMock(return_value=(True, ""))),
        patch("app.services.k3s_nodegroup.update_nodegroup", new=AsyncMock()) as update_nodegroup,
        patch("app.services.k3s_stampede._update_stampede_state", new=AsyncMock()) as update_state,
        patch("app.services.k3s_stampede._record_stampede_event", new=AsyncMock()) as record_event,
        patch("app.services.k3s_stampede._provision_and_track", new=AsyncMock(return_value=None)),
        patch("app.services.k3s_stampede.asyncio.create_task", side_effect=schedule_task),
    ):
...
async def test_scale_up_caps_binpacked_nodes_by_in_flight_capacity():
    ...
    with (
        patch(
            "app.services.k3s_stampede._get_stampede_state",
...
async def test_scale_up_blocks_gpu_nodes_when_quota_check_fails():
    ...
    with (
        ...
        patch(
            "app.services.k3s_stampede._check_gpu_quota_for_nodes",
            new=AsyncMock(return_value=(False, "quota exceeded")),
        ),
        patch("app.services.k3s_nodegroup.update_nodegroup", new=AsyncMock()) as update_nodegroup,
        patch("app.services.k3s_stampede._update_stampede_state", new=AsyncMock()) as update_state,
        patch("app.services.k3s_stampede._record_stampede_event", new=AsyncMock()) as record_event,
        patch("app.services.k3s_stampede.asyncio.create_task") as create_task,
    ):
        await _scale_up_nodegroup(
            cluster_id="cl-1",
            project_id="proj-1",
...
@pytest.mark.asyncio
async def test_scale_down_removes_node_with_only_daemonset_pods():
    import asyncio
    ...
    with (
        ...
        patch("app.services.k3s_stampede._update_stampede_state", new=AsyncMock()) as update_state,
        patch("app.services.k3s_stampede._record_stampede_event", new=AsyncMock()) as record_event,
        patch("app.services.k3s_autoscale.delete_nodegroup_vms", new=AsyncMock()) as delete_vms,
        patch("app.services.k3s_stampede.asyncio.create_task", side_effect=schedule_task),
    ):
        await _scale_down_nodegroup(
            cluster_id="cl-1",
    ...
    assert record_event.await_args.kwargs["status"] == "started"
    assert record_event.await_args.kwargs["extra"]["node_name"] == "node-a"
    delete_vms.assert_awaited_once_with(
        project_id="proj-1",
        cluster_id="cl-1",
        nodegroup_id="gpu-ng",
        vm_entries=[{"vm_id": "vm-a", "name": "node-a", "status": "ACTIVE"}],
    )


@pytest.mark.asyncio
async def test_provision_and_track_marks_partial_when_a_node_never_becomes_ready():
    from app.services.k3s_stampede import _provision_and_track
    ...
    with (
        patch(
            "app.services.k3s_autoscale.provision_nodegroup_vms",
            new=AsyncMock(
                return_value=[
        ...
        patch("app.services.k3s_kube.wait_node_ready", new=AsyncMock(side_effect=[True, False])),
        patch("app.services.k3s_nodegroup.update_nodegroup", new=AsyncMock()) as update_nodegroup,
        patch("app.services.k3s_nodegroup.get_nodegroup", new=AsyncMock(return_value=ng)),
        patch("app.services.k3s_nodegroup.set_nodegroup_count", new=AsyncMock()) as set_count,
        patch("app.services.k3s_stampede._update_stampede_state", new=AsyncMock()) as update_state,
        patch("app.services.k3s_stampede._record_stampede_event", new=AsyncMock()) as record_event,
    ):
...
async def test_provision_and_track_reconciles_missing_created_vms():
    ...
    with (
        patch("app.services.k3s_autoscale.provision_nodegroup_vms", new=AsyncMock(return_value=[])),
        patch("app.services.k3s_nodegroup.get_nodegroup", new=AsyncMock(return_value=ng)),
        patch("app.services.k3s_nodegroup.set_nodegroup_count", new=AsyncMock()) as set_count,
        patch("app.services.k3s_stampede._update_stampede_state", new=AsyncMock()) as update_state,
        patch("app.services.k3s_stampede._record_stampede_event", new=AsyncMock()) as record_event,
