...
async def test_rotate_certificates_emits_completed():
    ...
    from app.services.k3s_cert_rotation import rotate_certificates

    with (
        patch("app.services.k3s_cert_rotation.k3s_kube.list_server_nodes", new=AsyncMock(return_value=["node-1"])),
        patch("app.services.k3s_cert_rotation.k3s_kube.create_job", new=AsyncMock(return_value={})),
        ...
        patch("app.services.k3s_cert_rotation.k3s_db.record_rotation", new=AsyncMock()),
        patch("app.services.k3s_cert_rotation._invalidate_expiry_cache", new=AsyncMock()),
    ):
...
async def test_rotate_certificates_no_nodes_emits_failed():
    ...
    with patch("app.services.k3s_cert_rotation.k3s_kube.list_server_nodes", new=AsyncMock(return_value=[])):
        steps = [msg.step.value async for msg in rotate_certificates("c1", "proj1", "testuser")]
...
async def test_rotate_certificates_job_failure_emits_failed():
    ...
    with (
        patch("app.services.k3s_cert_rotation.k3s_kube.list_server_nodes", new=AsyncMock(return_value=["node-1"])),
        patch("app.services.k3s_cert_rotation.k3s_kube.create_job", new=AsyncMock(return_value={})),
        patch("app.services.k3s_cert_rotation.k3s_kube.wait_job_completed", new=AsyncMock(return_value=False)),
        patch("app.services.k3s_cert_rotation._invalidate_expiry_cache", new=AsyncMock()),
    ):
        steps = [msg.step.value async for msg in rotate_certificates("c1", "proj1", "testuser")]
...
async def test_rotate_certificates_node_not_ready_emits_failed():
    ...
    with (
        ...
        patch("app.services.k3s_cert_rotation.k3s_kube.create_job", new=AsyncMock(return_value={})),
        patch("app.services.k3s_cert_rotation.k3s_kube.wait_job_completed", new=AsyncMock(return_value=True)),
        patch("app.services.k3s_cert_rotation.k3s_kube.wait_node_ready", new=AsyncMock(return_value=False)),
        patch("app.services.k3s_cert_rotation._invalidate_expiry_cache", new=AsyncMock()),
    ):
        steps = [msg.step.value async for msg in rotate_certificates("c1", "proj1", "testuser")]
...
async def test_record_rotation_called_on_success():
    ...
    with (
        ...
        patch("app.services.k3s_cert_rotation.k3s_kube.wait_job_completed", new=AsyncMock(return_value=True)),
        patch("app.services.k3s_cert_rotation.k3s_kube.wait_node_ready", new=AsyncMock(return_value=True)),
        patch("app.services.k3s_cert_rotation.k3s_db.record_rotation", new=mock_record),
        patch("app.services.k3s_cert_rotation._invalidate_expiry_cache", new=AsyncMock()),
    ):
...
async def test_rotate_certs_sse_stream_returns_200(client):
    ...
    with (
        patch("app.api.k3s.certificates.k3s_db.get_cluster", new=AsyncMock(return_value=_make_cluster(master_count=3))),
        patch("app.api.k3s.certificates.acquire_rotation_lock", new=AsyncMock(return_value=True)),
