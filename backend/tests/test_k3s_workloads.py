...
from app.services.k3s_kube import (
    _deploy_from_k8s,
    _pod_from_k8s,
    _rs_from_k8s,
    _svc_from_k8s,
...
async def test_list_pods_endpoint(client):
    ...
    with (
        patch("app.api.k3s.pods.k3s_cluster") as mc,
        patch("app.api.k3s.pods.k3s_kube") as mk,
    ):
...
async def test_delete_pod_idempotent_404(client):
    ...
    with (
        patch("app.api.k3s.pods.k3s_cluster") as mc,
        patch("app.api.k3s.pods.k3s_kube") as mk,
        patch("app.api.k3s.pods.rec", new_callable=AsyncMock),
    ):
        mc.get_cluster = AsyncMock(return_value=_cluster())
...
async def test_restart_deployment_patches_annotation(client):
    ...
    with (
        patch("app.api.k3s.workloads.k3s_cluster") as mc,
        patch("app.api.k3s.workloads.k3s_kube") as mk,
        patch("app.api.k3s.workloads.rec", new_callable=AsyncMock),
    ):
        mc.get_cluster = AsyncMock(return_value=_cluster())
...
async def test_scale_deployment_negative_rejected(client):
    ...
    with (
        patch("app.api.k3s.workloads.k3s_cluster") as mc,
        patch("app.api.k3s.workloads.k3s_kube"),
    ):
        mc.get_cluster = AsyncMock(return_value=_cluster())
        resp = await client.patch(
            "/api/k3s/clusters/k3s-1/namespaces/default/deployments/mydeploy/scale",
...
async def test_list_services_endpoint(client):
    ...
    with (
        patch("app.api.k3s.k3s_services.k3s_cluster") as mc,
        patch("app.api.k3s.k3s_services.k3s_kube") as mk,
    ):
        mc.get_cluster = AsyncMock(return_value=_cluster())
