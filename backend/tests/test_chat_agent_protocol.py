...
from app.services.chat.agent_protocol import (
...
from app.services.chat.agent_runtime_v2 import dispatch_tool_call
from app.services.chat.tool_runtime import v2_builtin_tool_bindings
...
async def test_lumen_bindings_freeze_only_the_selected_grant_snapshot(monkeypatch):
    snapshot = tool_runtime.mcp_lumen.LumenGrantSnapshot(
    ...
    async def resolve(value):
        ...
        return type("Principal", (), {"grant_id": snapshot.grant_id, "scopes": frozenset({"mcp:read"})})()
    ...
    monkeypatch.setattr(tool_runtime.mcp_registry, "enabled_service_fingerprint", lambda: "services-fingerprint")
...
async def test_lumen_mutation_reuses_ledger_with_lumen_source(monkeypatch):
    ...
    async def claim(*_args, **kwargs):
        ...
        return tool_runtime.mcp_ledger.MutationClaim(state="claimed", invocation_id="invocation")
    ...
    monkeypatch.setattr(tool_runtime.mcp_registry, "build_mutation_preview", preview)
    monkeypatch.setattr(tool_runtime.mcp_registry, "dispatch", dispatch)
    monkeypatch.setattr(tool_runtime.mcp_registry, "output_payload", lambda *_args: {"status": "accepted"})
    monkeypatch.setattr(tool_runtime.mcp_ledger, "claim_mutation", claim)
    monkeypatch.setattr(tool_runtime.mcp_ledger, "authorize_mutation_dispatch", authorize)
    monkeypatch.setattr(tool_runtime.mcp_ledger, "complete_mutation", complete)
    ...
    assert sources == ["lumen", "lumen", "lumen"]
