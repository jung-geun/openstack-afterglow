...
async def test_admin_overview_allowed(admin_client):
    with patch("app.api.identity.admin.cached_call") as mc:
        mc.side_effect = AsyncMock(return_value={
            "hypervisor_count": 0, "running_vms": 0, "gpu_instances": 0,
            "instance_stats": {}, "vcpus": {"total": 0, "allowed": 0, "used": 0},
            "ram_gb": {"total": 0, "used": 0}, "disk_gb": {"total": 0, "used": 0},
            "containers_count": 0, "file_storage_count": 0,
        })
...
async def test_get_hypervisor_detail_allowed(admin_client, mock_conn):
    mock_conn.compute.get_hypervisor.return_value = MagicMock(
        id="hv-1", name="hv-1", status="enabled", state="up",
        host_ip="10.0.0.1", type="QEMU", vcpus=64, vcpus_used=0,
        memory_size=512000, memory_used=0, local_disk_size=5000, local_disk_used=0,
    )
...
async def test_create_port_requires_admin(non_admin_client):
    resp = await non_admin_client.post("/api/admin/ports", json={
        "network_id": "net-1", "name": "test-port"
    })
    assert resp.status_code == 403
