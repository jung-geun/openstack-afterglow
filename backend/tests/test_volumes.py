...
async def test_list_volumes(client, mock_conn):
    with patch("app.api.volumes.cinder.list_volumes", return_value=[make_mock_volume()]):
...
async def test_create_volume(client, mock_conn):
    with patch("app.api.volumes.cinder.create_empty_volume", return_value=make_mock_volume("vol-new", "new-vol")):
...
async def test_delete_volume(client, mock_conn):
    with patch("app.api.volumes.cinder.delete_volume", return_value=None):
...
async def test_get_volume(client, mock_conn):
    with patch("app.api.volumes.cinder.get_volume", return_value=make_mock_volume()):
