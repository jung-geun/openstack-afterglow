...
async def test_extend_volume_happy_path(client, mock_conn):
    ...
    with (
        ...
        resp = await client.post("/api/v1/volumes/vol-extend-1/extend", json={"new_size": 60})
...
async def test_extend_volume_smaller_size_returns_400(client, mock_conn):
    ...
    with (
        ...
        resp = await client.post("/api/v1/volumes/vol-extend-1/extend", json={"new_size": 30})
...
async def test_extend_volume_same_size_returns_400(client, mock_conn):
    ...
    with (
        ...
        resp = await client.post("/api/v1/volumes/vol-extend-1/extend", json={"new_size": 50})
...
async def test_extend_volume_not_found_returns_404(client, mock_conn):
    ...
    resp = await client.post("/api/v1/volumes/vol-missing/extend", json={"new_size": 60})
...
async def test_extend_volume_zero_size_returns_422(client):
    resp = await client.post("/api/v1/volumes/vol-extend-1/extend", json={"new_size": 0})
    assert resp.status_code == 422


@pytest.mark.asyncio
async def test_extend_volume_negative_size_returns_422(client):
    resp = await client.post("/api/v1/volumes/vol-extend-1/extend", json={"new_size": -10})
    assert resp.status_code == 422
