...
async def test_create_and_consume_ha_callback_token():
    ...
    with patch("app.services.k3s_cluster._get_client", return_value=mock_client):
        from app.services.k3s_cluster import consume_ha_callback_token, create_ha_callback_token
...
async def test_incr_ha_join_count():
    ...
    with patch("app.services.k3s_cluster._get_client", return_value=mock_client):
        from app.services.k3s_cluster import incr_ha_join_count
...
async def test_handle_ha_joiner_adds_lb_member_and_triggers_agents():
    ...
    with (
        patch("app.services.k3s_db.get_cluster", new=AsyncMock(return_value=cluster_info)),
        patch("app.services.k3s_db.incr_ha_join_count", new=AsyncMock(return_value=2)),
        patch("app.services.k3s_provisioner.provision_agents", new=AsyncMock()),
        patch("app.services.keystone.get_admin_connection_for_project") as mock_conn,
        patch("app.services.octavia.add_member") as mock_add,
        ...
        from app.api.k3s.callback import _handle_ha_joiner
        ...
        # provision_agents가 create_task로 스폰되었는지 확인
...
async def test_handle_ha_joiner_no_agents_if_not_all_joined():
    ...
    with (
        patch("app.services.k3s_db.get_cluster", new=AsyncMock(return_value=cluster_info)),
        patch("app.services.k3s_db.incr_ha_join_count", new=AsyncMock(return_value=1)),
        patch("app.services.keystone.get_admin_connection_for_project") as mock_conn,
        patch("app.services.octavia.add_member") as _,
        patch("asyncio.create_task") as mock_task,
    ):
        mock_conn.return_value = MagicMock()
