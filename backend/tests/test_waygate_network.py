...
from app.services import waygate_config, waygate_network
from app.services.waygate_network import WaygateNetworkError
...
def _db_available(monkeypatch):
    monkeypatch.setattr("app.api.waygate.attachments.is_db_available", lambda: True)
...
def _override_token_info(project_id: str = "test-project-123"):
    ...
    app.dependency_overrides[get_token_info] = _fn
...
class TestAttachNetworkApi:
    ...
    async def test_attach_404_when_server_not_owned(self, api_client):
        ...
        with patch("app.api.waygate.attachments.waygate_db") as db:
            db.get_server = AsyncMock(return_value=None)  # 없음/타 프로젝트 → 404
            resp = await api_client.post("/api/v1/waygate/servers/srv-x/networks", json={"network_id": _VALID_UUID})
    ...
    async def test_attach_422_on_invalid_network_id(self, api_client):
        ...
        resp = await api_client.post(
            "/api/v1/waygate/servers/srv-1/networks", json={"network_id": "not-a-uuid; rm -rf /"}
        )
    ...
    async def test_attach_422_on_invalid_nat_mode(self, api_client):
        _override_token_info()
        resp = await api_client.post(
            "/api/v1/waygate/servers/srv-1/networks",
    ...
    async def test_attach_success_201(self, api_client):
        _override_token_info()
        with (
            patch("app.api.waygate.attachments.waygate_db") as db,
            patch("app.api.waygate.attachments.waygate_network") as net,
        ):
            db.get_server = AsyncMock(return_value=_server())
            net.attach_network = AsyncMock(return_value=_attachment())
            resp = await api_client.post("/api/v1/waygate/servers/srv-1/networks", json={"network_id": _VALID_UUID})
...
class TestListDetachNetworkApi:
    ...
    async def test_list_returns_attachments(self, api_client):
        ...
        with patch("app.api.waygate.attachments.waygate_db") as db:
            ...
            resp = await api_client.get("/api/v1/waygate/servers/srv-1/networks")
    ...
    async def test_list_404_when_not_owned(self, api_client):
        ...
        with patch("app.api.waygate.attachments.waygate_db") as db:
            ...
            resp = await api_client.get("/api/v1/waygate/servers/srv-x/networks")
        assert resp.status_code == 404
    ...
    async def test_detach_success_204(self, api_client):
        ...
        with (
            ...
            resp = await api_client.delete("/api/v1/waygate/servers/srv-1/networks/1")
    ...
    async def test_detach_404_when_attachment_missing(self, api_client):
        ...
        with (
            ...
            resp = await api_client.delete("/api/v1/waygate/servers/srv-1/networks/999")
        assert resp.status_code == 404
