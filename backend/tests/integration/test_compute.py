...
async def test_get_image_not_found(client):
    resp = await client.get("/api/v1/images/nonexistent-id-12345")
...
async def test_keypair_crud(client):
    ...
    # 이전 잔여물 정리
    await client.delete(f"/api/v1/keypairs/{name}")

    # 생성
    resp = await client.post("/api/v1/keypairs", json={"name": name})
    ...
    # 삭제
    resp = await client.delete(f"/api/v1/keypairs/{name}")
...
async def test_get_instance_detail(client):
    """인스턴스가 있으면 상세 조회 테스트."""
    instances = (await client.get("/api/v1/instances")).json()
    if not instances:
...
async def test_get_instance_interfaces(client):
    ...
    instance_id = instances[0]["id"]
    resp = await client.get(f"/api/v1/instances/{instance_id}/interfaces")
    assert resp.status_code == 200
...
async def test_get_instance_volumes(client):
    ...
    instance_id = instances[0]["id"]
    resp = await client.get(f"/api/v1/instances/{instance_id}/volumes")
    assert resp.status_code == 200
    assert isinstance(resp.json(), list)
...
async def test_get_instance_security_groups(client):
    ...
    instance_id = instances[0]["id"]
    resp = await client.get(f"/api/v1/instances/{instance_id}/security-groups")
    assert resp.status_code == 200
    data = resp.json()
...
async def test_list_images_as_user(user_client):
    resp = await user_client.get("/api/v1/images")
    assert resp.status_code == 200
    assert isinstance(resp.json(), list)
...
async def test_list_flavors_as_user(user_client):
    resp = await user_client.get("/api/v1/flavors")
    assert resp.status_code == 200
    assert isinstance(resp.json(), list)
...
async def test_list_instances_as_user(user_client):
    resp = await user_client.get("/api/v1/instances")
    assert resp.status_code == 200
    assert isinstance(resp.json(), list)
