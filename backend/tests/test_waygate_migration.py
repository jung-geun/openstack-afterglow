...
from app.services import k3s_crypto, waygate_keys, waygate_migration
from app.services.waygate_migration import WaygateMigrationError
...
def _db_available(monkeypatch):
    monkeypatch.setattr("app.api.waygate.migration.is_db_available", lambda: True)
...
class TestMigrationApi:
    ...
    async def test_export_404_when_not_owned(self, api_client):
        ...
        with patch("app.api.waygate.migration.waygate_db") as db:
            db.get_server = AsyncMock(return_value=None)
            resp = await api_client.post("/api/v1/waygate/servers/srv-x/export", json={"passphrase": "passphrase-1"})
        assert resp.status_code == 404

    @pytest.mark.asyncio
    async def test_export_422_short_passphrase(self, api_client):
        _override_token_info()
        resp = await api_client.post("/api/v1/waygate/servers/srv-1/export", json={"passphrase": "short"})
    ...
    async def test_export_success(self, api_client):
        _override_token_info()
        with (
            patch("app.api.waygate.migration.waygate_db") as db,
            patch("app.api.waygate.migration.waygate_migration") as mig,
        ):
            ...
            resp = await api_client.post("/api/v1/waygate/servers/srv-1/export", json={"passphrase": "passphrase-1"})
    ...
    async def test_import_404_when_not_owned(self, api_client):
        ...
        with patch("app.api.waygate.migration.waygate_db") as db:
            ...
            resp = await api_client.post(
                "/api/v1/waygate/servers/srv-x/import",
    ...
    async def test_import_success_returns_summary(self, api_client):
        ...
        with (
            ...
            resp = await api_client.post(
                "/api/v1/waygate/servers/srv-1/import",
                json={"passphrase": "passphrase-1", "bundle": {"version": 1, "clients": []}},
            )
        assert resp.status_code == 200
