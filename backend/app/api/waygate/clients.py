...
from app.services import k3s_crypto, waygate_agent_auth, waygate_config, waygate_db, waygate_ipam, waygate_keys
...
async def list_waygate_clients(server_id: str, token_info: dict = Depends(get_token_info)):
    _require_db()
    project_id = token_info["project_id"]
...
async def delete_waygate_client(server_id: str, client_id: str, token_info: dict = Depends(get_token_info)):
    _require_db()
    project_id = token_info["project_id"]
    await _get_owned_server(project_id, server_id)
