...
from app.models.waygate import WaygateServerCreateRequest, WaygateServerInfo
from app.services import waygate_agent_auth, waygate_db
from app.services.waygate_provisioner import delete_waygate_server, provision_waygate_server
...
async def create_waygate_server(
    ...
    asyncio.create_task(
        provision_waygate_server(project_id, server_id, token_info.get("user_id", ""), token_info.get("username", ""))
    )
...
async def list_waygate_servers(token_info: dict = Depends(get_token_info)):
    _require_db()
    project_id = token_info["project_id"]
...
async def get_waygate_server(server_id: str, token_info: dict = Depends(get_token_info)):
    _require_db()
    project_id = token_info["project_id"]
    server = await waygate_db.get_server(project_id, server_id)
    if not server:
...
async def delete_waygate_server_endpoint(server_id: str, token_info: dict = Depends(get_token_info)):
    _require_db()
    project_id = token_info["project_id"]
    server = await waygate_db.get_server(project_id, server_id)
    if not server:
    ...
    asyncio.create_task(delete_waygate_server(project_id, server_id, token_info.get("user_id", "")))
