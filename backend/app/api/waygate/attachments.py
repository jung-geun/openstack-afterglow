...
from app.models.waygate import WaygateNetworkAttachCreateRequest, WaygateNetworkAttachmentInfo
from app.services import waygate_db, waygate_network
...
async def list_waygate_networks(server_id: str, token_info: dict = Depends(get_token_info)):
    _require_db()
    project_id = token_info["project_id"]
...
async def detach_waygate_network(server_id: str, attachment_id: int, token_info: dict = Depends(get_token_info)):
    _require_db()
    project_id = token_info["project_id"]
    server = await _get_owned_server(project_id, server_id)
    try:
