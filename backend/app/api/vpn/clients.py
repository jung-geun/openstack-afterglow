...
from app.models.vpn import (
    ...
    VpnClientCreateResponse,
    VpnClientInfo,
    VpnClientUpdateRequest,
)
from app.services import k3s_crypto, vpn_agent_auth, vpn_config, vpn_db, vpn_ipam, vpn_keys
...
async def _merge_client_status(client: dict, server_id: str) -> VpnClientInfo:
    info = VpnClientInfo(**{k: v for k, v in client.items() if k in VpnClientInfo.model_fields})
    status_result = await vpn_agent_auth.get_status_result(server_id)
    if status_result:
...
@router.post("/{server_id}/clients", status_code=201, response_model=VpnClientCreateResponse)
async def create_vpn_client(
    ...
    private_key, public_key = vpn_keys.generate_keypair()

    existing_clients = await vpn_db.list_clients(server_id, project_id)
    used_ips = [c["tunnel_ip"] for c in existing_clients]
    tunnel_ip = vpn_ipam.allocate_next_ip(server["tunnel_cidr"], used_ips)
    ...
    except VpnClientConflictError as exc:
        if exc.field == "name":
    ...
    tunnel_conf = vpn_config.render_client_conf(
    ...
    client = await vpn_db.get_client(server_id, project_id, client_id)
    info = await _merge_client_status(client, server_id)
    return VpnClientCreateResponse(**info.model_dump(), tunnel_conf=tunnel_conf)


@router.get("/{server_id}/clients", response_model=list[VpnClientInfo])
async def list_vpn_clients(server_id: str, token_info: dict = Depends(get_token_info)):
    _require_db()
    project_id = token_info["project_id"]
    await _get_owned_server(project_id, server_id)
    clients = await vpn_db.list_clients(server_id, project_id)
    return [await _merge_client_status(c, server_id) for c in clients]


@router.patch("/{server_id}/clients/{client_id}", response_model=VpnClientInfo)
async def update_vpn_client(
    ...
    project_id = token_info["project_id"]
    await _get_owned_server(project_id, server_id)
    updated = await vpn_db.update_client(server_id, project_id, client_id, name=body.name, enabled=body.enabled)
    if not updated:
...
async def delete_vpn_client(server_id: str, client_id: str, token_info: dict = Depends(get_token_info)):
    _require_db()
    project_id = token_info["project_id"]
    await _get_owned_server(project_id, server_id)
    ok = await vpn_db.soft_delete_client(server_id, project_id, client_id, token_info.get("user_id", ""))
    if not ok:
        raise HTTPException(status_code=404, detail="VPN 클라이언트를 찾을 수 없습니다")


@router.get("/{server_id}/clients/{client_id}/config")
async def download_vpn_client_config(server_id: str, client_id: str, token_info: dict = Depends(get_token_info)):
