...
async def get_dashboard_summary(
    ...
    flavors_by_id: dict = {f["id"]: f["name"] for f in all_flavors}

    gpu_used = 0
    for s in servers:
        if s["status"] != "ACTIVE":
            continue
        name = flavors_by_id.get(s.get("flavor_id", ""), "")
        gpu_used += _gpu_count_from_name(name)
