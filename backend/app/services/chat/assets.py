...
from app.models.chat_contracts import UserAssetInputPart, UserTextInputPart, validate_user_input_parts
from app.services.chat.conversation_store import ChatStorageUnavailable
...
def _inspect_with_bounded_child(path: Path) -> tuple[str, dict[str, int]]:
    try:
        result = subprocess.run(
            [sys.executable, "-m", "app.services.chat.asset_inspector", str(path)],
            check=False,
