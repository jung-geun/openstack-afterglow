...
from app.services import neutron, nova, vpn_agent_auth, vpn_config, vpn_db
from app.services.builder_vm import _allocate_new_fip, _extract_fixed_ip, _wait_for_active

_logger = logging.getLogger(__name__)

_WG_INGRESS_SG_NAME = "afterglow-vpn-wireguard"
...
def _resolve_flavor_id(conn, flavor_name: str, override_flavor_id: str) -> str:
    ...
    for f in flavors:
        if f.name == flavor_name:
            return f.id
    raise RuntimeError(f"flavor '{flavor_name}' 를 찾을 수 없습니다 (config.toml [vpn] flavor_id 로 override 가능)")
...
def _ensure_wireguard_sg(conn, project_id: str, listen_port: int) -> str:
    ...
    if existing is None:
        sg = neutron.create_security_group(
            conn,
            sg_name,
            f"WireGuard VPN gateway UDP ingress {neutron.AFTERGLOW_MANAGED_TAG}",
        )
        sg_id = sg["id"]
...
async def provision_vpn_server(
    project_id: str,
    server_id: str,
    user_id: str,
    username: str,
    ...
    try:
        ...
        await vpn_db.update_server_status(server_id, "ERROR", f"OpenStack 연결 실패: {e}")
        return

    try:
        server_record = await vpn_db.get_server_by_id(server_id)
        if not server_record:
            _logger.error("vpn_provisioner: server %s not found in DB", server_id)
            return

        name = server_record["name"]
        listen_port = server_record["listen_port"]

        provider_network_id = settings.vpn_provider_network_id
        if not provider_network_id:
            raise RuntimeError("config.toml [vpn] provider_network_id 가 설정되지 않았습니다")

        flavor_id = await asyncio.to_thread(_resolve_flavor_id, conn, settings.vpn_flavor_name, settings.vpn_flavor_id)
        ...
        bootstrap_token = await vpn_agent_auth.issue_report_token(server_id, project_id)

        callback_base = settings.vpn_callback_base_url.rstrip("/")
        register_url = f"{callback_base}/api/v1/vpn/servers/{server_id}/agent/register"
        desired_state_url = f"{callback_base}/api/v1/vpn/servers/{server_id}/agent/desired-state"
        status_url = f"{callback_base}/api/v1/vpn/servers/{server_id}/agent/status"

        userdata = vpn_config.render_agent_userdata(
            server_name=name,
        ...
        image_id = settings.vpn_image_id
        if not image_id:
            raise RuntimeError("config.toml [vpn] image_id 가 설정되지 않았습니다")
        ...
        server = await asyncio.to_thread(
            ...
            user_data=userdata,
            key_name=settings.vpn_key_name or None,
            metadata={"afterglow_managed": "true", "vpn_type": "gateway", "vpn_server_id": server_id},
        )
        ...
        if not fixed_ip:
            raise RuntimeError(f"VPN VM {server.id}: fixed IP를 찾을 수 없습니다")

        # 6. endpoint IP 결정 — floating_network_id 설정 시 FIP, 아니면 fixed IP 그대로 사용
        #    (builder_vm.create_ephemeral_vm 233-238행과 동일 분기)
        if settings.vpn_floating_network_id:
            fip_addr, fip_id = await _allocate_new_fip(conn, server.id, settings.vpn_floating_network_id)
        ...
        _logger.info(
            "vpn_provisioner: server %s VM active (vm_id=%s, endpoint=%s), waiting for agent register",
            server_id,
            server.id,
            endpoint_ip,
        )
        ...
        _logger.error("vpn_provisioner: 프로비저닝 실패 (server=%s): %s", server_id, e, exc_info=True)
        await _rollback(
            conn,
        ...
        await vpn_db.update_server_status(server_id, "ERROR", f"프로비저닝 실패: {e}")
    finally:
        try:
...
async def delete_vpn_server(project_id: str, server_id: str, user_id: str) -> None:
    """VPN 서버 삭제 백그라운드 태스크. 소유권 검증은 호출부(API)에서 이미 수행됨."""
    from app.services import keystone

    server_record = await vpn_db.get_server(project_id, server_id)
    if not server_record:
        _logger.warning("vpn_provisioner: delete 대상 server %s not found (project=%s)", server_id, project_id)
        return

    try:
        conn = await asyncio.to_thread(keystone.get_admin_connection_for_project, project_id)
    except Exception as e:
        _logger.error("vpn_provisioner: delete 시 OpenStack 연결 실패: %s", e)
        await vpn_db.update_server_status(server_id, "ERROR", f"삭제 중 연결 실패: {e}")
        return
    ...
    try:
        ...
        if vm_id:
            ...
            try:
                ...
                _logger.warning("vpn_provisioner: VM %s 삭제 대기 타임아웃 — 계속 진행", vm_id)
        ...
        if fip_id:
            try:
                ...
                _logger.warning("vpn_provisioner: FIP %s 삭제 실패", fip_id, exc_info=True)

        port_id = server_record.get("provider_port_id")
        if port_id:
            await asyncio.to_thread(neutron.delete_port, conn, port_id)

        await vpn_agent_auth.revoke_report_token_by_server(server_id)
        await vpn_db.soft_delete_server(project_id, server_id, user_id, reason="사용자 요청")
        _logger.info("vpn_provisioner: server %s 삭제 완료", server_id)
    except Exception as e:
        _logger.error("vpn_provisioner: 삭제 실패 (server=%s): %s", server_id, e, exc_info=True)
        await vpn_db.update_server_status(server_id, "ERROR", f"삭제 실패: {e}")
    finally:
        try:
            await asyncio.to_thread(conn.close)
        except Exception:
