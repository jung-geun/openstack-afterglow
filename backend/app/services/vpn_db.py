...
from app.models.db import VpnClient, VpnServer

_logger = logging.getLogger(__name__)


class VpnClientConflictError(Exception):
...
def _server_to_dict(s: VpnServer) -> dict:
    return {
        "id": s.id,
        ...
        "server_vm_id": s.server_vm_id,
        "flavor_id": s.flavor_id,
        "provider_network_id": s.provider_network_id,
        "provider_port_id": s.provider_port_id,
...
def _client_to_dict(c: VpnClient) -> dict:
    return {
        "id": c.id,
...
async def get_server(project_id: str, server_id: str) -> dict | None:
    ...
    async with factory() as session:
        stmt = select(VpnServer).where(
            VpnServer.id == server_id, VpnServer.project_id == project_id, VpnServer.deleted_at.is_(None)
        )
...
async def list_servers(project_id: str) -> list[dict]:
    ...
    async with factory() as session:
        stmt = (
            select(VpnServer)
            .where(VpnServer.project_id == project_id, VpnServer.deleted_at.is_(None))
...
async def get_server_by_id(server_id: str) -> dict | None:
    ...
    factory = get_session_factory()
    async with factory() as session:
        stmt = select(VpnServer).where(VpnServer.id == server_id, VpnServer.deleted_at.is_(None))
        result = await session.execute(stmt)
        server = result.scalar_one_or_none()
        if server is None:
            return None
...
async def update_server_status(
    ...
    **extra_fields,
    ...
    factory = get_session_factory()
    async with factory() as session:
        stmt = select(VpnServer).where(VpnServer.id == server_id)
        result = await session.execute(stmt)
        server = result.scalar_one_or_none()
        if server is None:
...
async def create_client_record(server_id: str, project_id: str, client_id: str, data: dict) -> None:
    ...
    async with factory() as session:
        ...
        try:
            ...
            _logger.warning("VPN 클라이언트 생성 unique 충돌 (server=%s, field=%s)", server_id, field or "unknown")
            raise VpnClientConflictError(field) from exc


def _classify_unique_violation(exc: IntegrityError) -> str | None:
    """IntegrityError 메시지에서 위반된 제약(name vs tunnel_ip)을 판별한다.

    MySQL/MariaDB는 "uq_vpn_client_server_name" 같은 제약명을, SQLite는
    "vpn_clients.server_id, vpn_clients.name" 같은 컬럼명을 에러 텍스트에 포함하므로
    두 형태 모두 텍스트 매칭으로 분기한다. 판별 불가 시 None(호출부가 일반 메시지 사용).
    """
    text = str(exc.orig or exc).lower()
    if "uq_vpn_client_server_name" in text or "vpn_clients.name" in text:
        return "name"
    if "uq_vpn_client_server_tunnel_ip" in text or "vpn_clients.tunnel_ip" in text:
        return "tunnel_ip"
    return None
    ...
    async with factory() as session:
        stmt = (
            ...
            .where(
                ...
                VpnClient.project_id == project_id,
                VpnClient.deleted_at.is_(None),
            )
...
async def list_all_active_clients(server_id: str) -> list[dict]:
    ...
    factory = get_session_factory()
    async with factory() as session:
        stmt = select(VpnClient).where(VpnClient.server_id == server_id, VpnClient.deleted_at.is_(None))
        result = await session.execute(stmt)
        return [_client_to_dict(c) for c in result.scalars().all()]
