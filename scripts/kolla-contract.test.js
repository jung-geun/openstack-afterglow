...
test("Drover and Lumen Kolla role packaging, operator specifications, and installer contracts", () => {
	...
	assert.match(
		pyproject,
		/lumen-kolla @ https:\/\/github\.com\/openstack-afterglow\/lumen\/releases\/download\/v0\.1\.5\/lumen_kolla-0\.1\.5-py3-none-any\.whl#sha256=18750bb7e548d8a4910dac400af564d01801b9dea0a1adaa5a8cf8a990924c55/
	)
	assert.match(
		pyproject,
...
test("Hermetic integration test for drover-kolla and lumen-kolla wheel installation, Ansible dispatch, and uninstall safety", () => {
	...
	const lumenDistInfoDir = path.join(rolesDir, "lumen_kolla-0.1.5.dist-info")
	...
	try {
		// 1. Setup mock installed drover-kolla 0.2.19 and lumen-kolla 0.1.5 wheel files in Python share/roles
		...
		assert.match(fs.readFileSync(path.join(lumenDistInfoDir, "METADATA"), "utf8"), /Version: 0\.1\.5/)
