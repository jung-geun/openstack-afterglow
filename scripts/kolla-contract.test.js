...
test("Afterglow hands operator TOML to containers without surrendering Kolla-owned settings", () => {
	...
	assert.match(configTask, /tomllib\.load\(open\(sys\.argv\[1\], 'rb'\)\)/)
	assert.match(configTask, /Config \| Copy operator configuration override/)
	assert.match(configTask, /dest: "\{\{ afterglow_config_dir \}\}\/\{\{ afterglow_operator_config_name \}\}"/)
	assert.match(configTask, /src: afterglow\.kolla\.conf\.j2/)
	assert.equal((configTask.match(/no_log: true/g) || []).length >= 4, true)
	...
	assert.match(sample, /^afterglow_operator_config_source: "\/etc\/kolla\/afterglow\/operator\/afterglow\.operator\.conf"$/m)
})