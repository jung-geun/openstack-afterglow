...
test("Drover and Lumen Kolla role packaging, operator specifications, and installer contracts", () => {
	...
	assert.match(
		pyproject,
		/lumen-kolla @ https:\/\/github\.com\/openstack-afterglow\/lumen\/releases\/download\/v0\.1\.2\/lumen_kolla-0\.1\.2-py3-none-any\.whl#sha256=7c164144495635f6c6c062632c44eb096a9cca0692ec32af4f5f290fbeab6570/
	)
	assert.match(
		pyproject,
...
test("Hermetic integration test for drover-kolla and lumen-kolla wheel installation, Ansible dispatch, and uninstall safety", () => {
	...
	const lumenDistInfoDir = path.join(rolesDir, "lumen_kolla-0.1.2.dist-info")
	...
	try {
		// 1. Setup mock installed drover-kolla 0.2.19 and lumen-kolla 0.1.2 wheel files in Python share/roles
		...
		assert.match(fs.readFileSync(path.join(lumenDistInfoDir, "METADATA"), "utf8"), /Version: 0\.1\.2/)
